...
@interface EVSVideoPlayerProgressSyncRequestHeader:NSObject
@property(copy,nonatomic) NSString *name;
@property(copy,nonatomic) NSString *request_id;
@end

@interface EVSVideoPlayerProgressSyncRequestPayload : NSObject
/**
 *  STARTED:播放开始播放的时候，发送此类型并告知 resource_id，手机端可获得播放内容信息
 *  FAILED:播放失败的时候，发送此类型并告知播放失败的 resource_id，云端会下发其他内容
 *  NEARLY_FINISHED:音频播放即将结束，一般取内容的1/3的时间长度 或 结束前10秒或播放后30秒（不是必传）
...
@interface EVSVideoPlayerProgressSyncRequest : NSObject
@property(strong,nonatomic) EVSVideoPlayerProgressSyncRequestHeader *header;
@property(strong,nonatomic) EVSVideoPlayerProgressSyncRequestPayload *payload;
@end

@interface EVSVideoPlayerProgressSync : EVSBaseProtocolModel
@property(strong,nonatomic) EVSVideoPlayerProgressSyncRequest *iflyos_request;//播放器信息同步
@end
